//
//  AwardVotingRow.swift
//  BaseballStats
//
//  Shared rendering for an award-voting leaderboard — the ranked row (with its
//  adaptive batting/pitching/two-way stat subtitle) and the list wrapper that
//  hosts the rows. Extracted so BOTH the fixed-combo sheet (`AwardVotingView`,
//  reached from the career Awards section and Team History) and the filtered
//  browser (`AwardVotingBrowserView` in Search) render identically without
//  duplicating the stat-formatting logic.
//
...
/// The ranked voting list: tappable rows that push to a player profile.
/// `.insetGrouped` over a hidden scroll background so a parent gradient shows
/// through — identical chrome to the original `AwardVotingView` list.
struct AwardVotingResultsList: View {
...
/// Single voting row — rank badge + player name (with adaptive stat-line
/// subtitle) + points-won total + first-place vote count. Visual hierarchy
/// parallels the LeaderboardRow so the two list surfaces feel related.
struct AwardVotingRow: View {
